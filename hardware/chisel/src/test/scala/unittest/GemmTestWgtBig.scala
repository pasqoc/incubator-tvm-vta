...
class TestTensorGemmWgtBig(c: TensorGemm, commonSize: Int, inpSize: Int, wgtSize: Int, gemmCommonSize: Int, gemmInpSize: Int, gemmWgtSize: Int) extends GemmTestGEMMAlgorithms(c, gemmCommonSize, gemmInpSize, gemmWgtSize) {
  ...
  println(s"-I- Binning: WxH inp ${commonBSize}x${inpBSize} wgt ${commonBSize}x${wgtBSize} acc ${wgtBSize}x${inpBSize}.")
  ...
  for (wgtBRowIdx  <- 0 until wgtBSize) {
    for (wgtBColIdx  <- 0 until commonBSize) {
      memManager.readSubArray(memB, tnzrB, wgtBRowIdx * gemmWgtSize, 
                   wgtBColIdx * gemmCommonSize, gemmCommonSize, gemmWgtSize)
      numWgtReads += 1
      val inpBColIdx = wgtBColIdx
      for (inpBRowIdx  <- 0 until inpBSize) {
        memManager.readSubArray(memA, tnzrA, inpBRowIdx * gemmInpSize, 
                     inpBColIdx * gemmCommonSize, gemmCommonSize, gemmInpSize)
        numInpReads += 1
  ...
  println(s"-I- Wgt Mode. Memory transfer Nb:Total Bytes: Inp: ${numInpReads}:${numInpReads*gemmCommonSize*gemmInpSize} Wgt: ${numWgtReads}:${numWgtReads*gemmCommonSize*gemmWgtSize} Acc: read=${numAccReads}:${numAccReads*gemmInpSize*gemmWgtSize*4} write=${numAccWrites}:${numAccWrites*gemmInpSize*gemmWgtSize*4}")
...
class TensorGemmWgtTestBig32x32x32 extends GenericTest( "TensorGemm_Wgt_Big_32", (p:Parameters) => new TensorGemm()(p), (c:TensorGemm) => new TestTensorGemmWgtBig(c, 32, 32, 32, 16, 16, 16))
//class TensorGemmWgtTestBig1024x1024x1024 extends GenericTest( "TensorGemm_Wgt_Big_1024", (p:Parameters) => new TensorGemm()(p), (c:TensorGemm) => new TestTensorGemmWgtBig(c, 1024, 1024, 1024, 32, 32, 32))
//class TensorGemmWgtTestBig192x12544x64 extends GenericTest( "TensorGemm_Wgt_Big_32", (p:Parameters) => new TensorGemm()(p), (c:TensorGemm) => new TestTensorGemmWgtBig(c, 192, 12544, 64, 64, 64, 64))
