...
object JSONMaxBurstLength {
  def apply( mp : vta.interface.axi.AXIParams) = {
    val default_result = 1 << mp.lenBits
    JSONConfig.m.get( "MAX_BURST_LENGTH") match {
...
class TensorLoadNarrowVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  val waddrTensInstrPipe = VecInit((for (j <- 0 until splitFactorL1) yield {
    ShiftRegister(waddrTensInstrTmp, if (writeDelay > 0) 1 else 0)
  }).flatMap(elem => for (k <- 0 until splitFactorL0) yield {
    elem
  }).flatMap(elem => for (k <- 0 until splitMemBlockFactor) yield {
    ShiftRegister(elem, if (writeDelay < 2) 0 else writeDelay - 1)
  }))
  ...
  for (j <- 0 until memSizeRatio * splitMemBlockFactor) {
    wen(j) := Mux(
      ShiftRegister(state === sIdle, writeDelay, resetData = true.B, en = true.B),
      wenDirect(j),
      ShiftRegister(wenTensInstr(j), writeDelay))
  }
  ...
  val wdataTensInstrDataPipe = VecInit((for (j <- 0 until splitFactorL0) yield {
    ShiftRegister(vmeDataBitsPipe.data, if (writeDelay > 0) 1 else 0)
    ...
    ShiftRegister(
      ...
      if (writeDelay < 2) 0 else writeDelay - 1)
  }))
  ...
  for (j <- 0 until memSizeRatio * splitMemBlockFactor) {
    ...
    val padValue = Mux(RegNext(dec.is_min_pad_value, init = false.B), Fill(tp.memBlockBits/tp.tensorElemBits,
      (BigInt(1) << (tp.tensorElemBits-1)).asUInt(tp.tensorElemBits.W)), 0.U)

    wdataTensInstr(j) := Mux(
      ShiftRegister(isZeroPadWrite, writeDelay, resetData = false.B, en = true.B),
      ShiftRegister(padValue /* a single group total data bits */, writeDelay),
  ...
  for (j <- 0 until memSizeRatio * splitMemBlockFactor) {
    when(wen(j)) {
      ...
      if (p(VerifKey).trace) {
        trace_mgr.Event("+LD_SRAM", " %x ", ShiftRegister(wdata(j), writeDelay, en = true.B))
        when (rdDataDestCol === (memSizeRatio - 1).U)  {
  ...
  for (grIdx <- 0 until splitDataFactor) {
    val rvalid = ShiftRegister(io.tensor.rd(grIdx).idx.valid, tp.readIdxDelay + 1, resetData = false.B, en = true.B)
    ...
    io.tensor.rd(grIdx).data.bits :=
      VecInit(for (memBlkIdx <- 0 until memsInGroup) yield {
        tensorFile(grIdx * memsInGroup + memBlkIdx).read(
          ShiftRegister(io.tensor.rd(grIdx).idx.bits, tp.readIdxDelay),
          ShiftRegister(io.tensor.rd(grIdx).idx.valid, tp.readIdxDelay, resetData = false.B, en = true.B))
...
class ReadVMEData(tensorType: String = "none", debug: Boolean = false)(
  ...
  when(io.vmeData.fire()) {
    ...
    if (p(VerifKey).trace) {
      when(rdDataDestCol === rdDataCol && rdDataDestIdx === rdDataIdx) {
        trace_mgr.Event(s"LD_SRAM ${tensorType.toUpperCase}", " %x %x", rdDataDestIdx, rdDataDestCol)
      }
    }
  }
...
class GenVMECmd(tensorType: String = "none", debug: Boolean = false)(
  ...
  val maxTransfer = JSONMaxBurstLength(mp).U
  // from old data ctrl
