...
class TensorStoreWideVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  val localStart = ShiftRegister(io.start, writeDelay, resetData = false.B, en = true.B)
  ...
  for (grpIdx <- 0 until splitDataFactor) {
    ...
    for (i <- 0 until tp.clSizeRatio) {
      when(ShiftRegister(io.tensor.wr(grpIdx).valid && directWrTensorIdx === i.U, writeDelay,
        resetData = false.B, en = true.B)) {

        tensorFile(i*splitDataFactor + grpIdx).write(ShiftRegister(directWrIdx, writeDelay),
          ShiftRegister(io.tensor.wr(grpIdx).bits.data.asUInt, writeDelay))
      }
    }
  }
  ...
  //--- Read memory ---
  //--------------------
  // first pulse doesnt reead whole data size, it is bounded DRAM data alignment
  // ! - data pulse boundary
  // . - tenzor boundary
  // tz - not used
  // TZ - tensor to store
  ...
  for (grpIdx <- 0 until splitDataFactor) {
    if (p(VerifKey).trace) {
      when(io.tensor.wr(grpIdx).valid) {
        trace_mgr.Event("ST_SRAM", "%x", io.tensor.wr(grpIdx).bits.idx)
        io.tensor.wr(grpIdx).bits.data.foreach { tensor =>
          tensor.foreach { elem =>
            trace_mgr.Event("+ST_SRAM", " %x", elem)
          }
        }
  ...
  if (debug) {
    ...
    //    ysize, ycnt, spReadAddr, waddr_cur, xlen, xrem)
    //}
    when(io.vme_wr.data.fire()) {
