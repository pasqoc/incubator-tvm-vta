...
class TensorParams(tensorType: String = "none")(implicit p: Parameters) extends Bundle {
  ...
  // This is a per macro memory
  val writeDelay =
    if (tensorType == "inp") {
      JSONCoreOptions.pipeDelayWrInp() // VME data load write (per group)
    } else if (tensorType == "wgt") {
      JSONCoreOptions.pipeDelayWrWgt() // VME data load write (per group)
    } else if (tensorType == "acc") {
      JSONCoreOptions.pipeDelayWrAcc() // VME data load write (per group)
    } else if (tensorType == "fetch") {
      0
    } else if (tensorType == "uop") {
      0
    } else if (tensorType == "out") {
      JSONCoreOptions.pipeDelayWrOut() // direct write from core
    } else {
      0
    }
  ...
  val readIdxDelay =
    if (tensorType == "inp") {
      JSONCoreOptions.pipeDelayGemmInpIdx() // GEMM data read (per memsplit)
    } else if (tensorType == "wgt") {
      0
    } else if (tensorType == "acc") {
      0
  ...
  val vmeDataDeleay =
    if (tensorType == "inp") {
      JSONCoreOptions.pipeDelayVmeInp() // VME data signals delay
    } else if (tensorType == "wgt") {
      JSONCoreOptions.pipeDelayVmeWgt() // VME data signals delay
    } else if (tensorType == "acc") {
      JSONCoreOptions.pipeDelayVmeAcc()  // VME data signals delay
    } else if (tensorType == "fetch") {
      0
    } else if (tensorType == "uop") {
      JSONCoreOptions.pipeDelayVmeUop() // VME data signals delay
    } else if (tensorType == "out") {
      0
    } else {
      0
  ...
  val splitMemsFactor =
    if (tensorType == "inp") {
      JSONCoreOptions.inpSpSplitFactor()
    } else {
      1
    }
  ...
  def paramsStr () = {
    ...
    s" split width:${splitWidth} pipe write:${writeDelay}"
  }
}
