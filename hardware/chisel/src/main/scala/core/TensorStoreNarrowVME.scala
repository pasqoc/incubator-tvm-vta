...
class TensorStoreNarrowVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  for (i <- 0 until tensorLength) {
    ...
    when(ShiftRegister(io.tensor.wr(0).valid, writeDelay, resetData = false.B, en = true.B)) {
      tensorFile(i).write(ShiftRegister(io.tensor.wr(0).bits.idx, writeDelay),
      ShiftRegister(inWrData, writeDelay), no_mask)
    }
  }
  ...
  if (p(VerifKey).trace) {
    when(io.tensor.wr(0).valid) {
      trace_mgr.Event("ST_SRAM", "%x", io.tensor.wr(0).bits.idx)
      io.tensor.wr(0).bits.data.foreach { tensor =>
        tensor.foreach { elem =>
