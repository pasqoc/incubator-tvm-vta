...
class TensorLoadWideVME(tensorType: String = "none", debug: Boolean = false)(
  ...
  val vmeDataDeleay = tp.vmeDataDeleay
  val vmeDataBitsPipe = ShiftRegister(io.vme_rd.data.bits, vmeDataDeleay, en = true.B)
  val vmeDataValidPipe = ShiftRegister(io.vme_rd.data.valid, vmeDataDeleay, resetData = false.B, en = true.B)
  val vmeDataReadyPipe = ShiftRegister(io.vme_rd.data.ready, vmeDataDeleay, resetData = true.B, en = true.B)
    ...
    for (grpIdx <- 0 until splitDataFactor) {
      for (memIdx <- 0 until splitMemFactor) { // duplicate control
        wmask(i*splitDataFactor*splitMemFactor + grpIdx * splitMemFactor + memIdx) :=
          Mux(
            ShiftRegister(state === sIdle, writeDelay, resetData = true.B, en = true.B),
            directWrMask(grpIdx)(i),
            Mux(
              ShiftRegister(isZeroPadWrite, writeDelay, resetData = false.B, en = true.B),
              ShiftRegister(zpDestMask(i), writeDelay),
              Mux(
                ShiftRegister(vmeDataFirePipe, writeDelay, resetData = false.B, en = true.B),
                ShiftRegister(rdDataWrEn(i), writeDelay),
                false.B)))
      }
    }
  }
  ...
  for (i <- 0 until tensorsInClNb){
    for (grpIdx <- 0 until splitDataFactor) {
      val zpDestData = Mux(
        RegNext(dec.is_min_pad_value, init = false.B),
          Fill(
            tp.tensorSizeBits/tp.tensorElemBits,
            (BigInt(1) << (tp.tensorElemBits-1)).asUInt(tp.tensorElemBits.W)),
          0.U)
      wdata(i*splitDataFactor + grpIdx) := Mux(
        ...
        Mux(
          ShiftRegister(isZeroPadWrite, writeDelay, resetData = false.B, en = true.B),
          ShiftRegister(zpDestData /* group size zero */, writeDelay),
          ShiftRegister((rdDataWrData(i).asTypeOf(Vec(splitDataFactor, UInt(groupSizeBits.W))))(grpIdx), writeDelay)))
    }
  }
      ...
      for (memIdx <- 0 until splitMemFactor) { // duplicate control
        widx(i*splitDataFactor*splitMemFactor + grpIdx * splitMemFactor + memIdx) :=
          Mux(
            ...
            Mux(
              ShiftRegister(isZeroPadWrite, writeDelay, resetData = false.B, en = true.B),
              ShiftRegister(zpDestIdx, writeDelay),
              ShiftRegister(rdDataWrIdx(i), writeDelay)))
      }
    }
  }
  ...
  for (i <- 0 until tensorsInClNb) {
    if (p(VerifKey).trace) {
      when(state =/= sIdle) {
        when (wmask(i)) {
          trace_mgr.Event("+LD_SRAM", " TODO: Reimplement")
        }
      }
    }
    for (grpIdx <- 0 until splitDataFactor) {
      ...
      VecInit(for (memIdx <- 0 until splitMemFactor) yield {
        tensorFile(
          i*splitDataFactor*splitMemFactor + grpIdx * splitMemFactor + memIdx).read(
            ShiftRegister(rIdx, tp.readIdxDelay),
            ShiftRegister(VecInit(rMask.asBools)(i), tp.readIdxDelay, resetData = false.B, en = true.B))
      }).asUInt
    }

    val rdata = Wire(UInt(tp.tensorSizeBits.W))
    rdata := Mux1H(ShiftRegister(rMask, tp.readIdxDelay + 1), rdataVec)
    io.tensor.rd(grpIdx).data.bits := rdata.asTypeOf(io.tensor.rd(grpIdx).data.bits.cloneType)

    val rvalid = ShiftRegister(io.tensor.rd(grpIdx).idx.valid, tp.readIdxDelay + 1, resetData = false.B, en = true.B)
  ...
  io.done := ShiftRegister(localDone, writeDelay, resetData = false.B, en = true.B)
}
...
class GenVMECmdWide(tensorType: String = "none", debug: Boolean = false)(
  ...
  //val rdLen1stTrans = Wire(Uint((io.xsize.getWidth + log2Ceil(elemBytes) - log2Ceil(clBytes)).W))
  //val maxTransfer = (1 << mp.lenBits).U // max number of pulses in transfer
  val maxTransfer = JSONMaxBurstLength(mp).U // max number of pulses in transfer
