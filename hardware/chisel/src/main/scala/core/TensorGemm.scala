...
class TensorGemmPipelinedSplit (implicit p: Parameters) extends TensorGemmIfc {
  ...
  // additional pipe latency of wgt/inp read
  val scratchpadReadLatency = JSONCoreOptions.pipeDelayGemmRdInpWgt()
  val inpReadIdxLatency = JSONCoreOptions.pipeDelayGemmInpIdx()
  val uopReadLatency = JSONCoreOptions.pipeDelayGemmRdUop()
...
// comment to  split write out
  ...
  // trace
  if (p(VerifKey).trace) {
    // This is an example of cross module reference using the boring utility.
    val cnt_o = WireInit(chiselTypeOf(io.dec.lp_0), 0.U)
    val cnt_i = WireInit(chiselTypeOf(io.dec.lp_1), 0.U)
    val uop_x = WireInit(chiselTypeOf(m.uop_idx), 0.U)
    BoringUtils.bore(m.cnt_o, Seq(cnt_o))
    BoringUtils.bore(m.cnt_i, Seq(cnt_i))
    BoringUtils.bore(m.uop_idx, Seq(uop_x))
    // To make sense of an event in a pipeline we need to present data that
    // refers to the stage we are reporting. Sometimes we will not have all
    // signals to report available and therefore we will have to pipe them
    // explicitly if needed.
    val cnt_o_5 = ShiftRegister(cnt_o, 5)
    val cnt_i_5 = ShiftRegister(cnt_i, 5)
    val uop_x_5 = ShiftRegister(uop_x, 5)

    when(io.acc.wr(io.acc.closestIOGrpIdx).valid) {
      // A trace event takes a unique name and a format + values in the styel of
      // a printf call.
      trace_mgr.Event("GEM_ITR", "%x %x %x %x",
      cnt_o_5, cnt_i_5, uop_x_5, io.acc.wr(0).bits.idx)
      io.acc.wr(0).bits.data.foreach { tensor =>
        tensor.foreach { elem =>
          // To add to the same event line make a continuation call with the same
          // event name prepended with a '+'.
          //trace_mgr.Event("+GEM_ITR", " %x", elem(7, 0))
          trace_mgr.Event("+GEM_ITR", " %x", elem)
        }
      }
      // If needed terminate a trace line with a newline.
      trace_mgr.Event("+GEM_ITR", "\n")
    }

    when(state === sIdle && io.start) {
      trace_mgr.Event("GEM_LOOP", "%x %x %x %x\n",
      io.dec.lp_0, io.dec.lp_1, io.dec.uop_begin, io.dec.uop_end)
    }
  }
}

//class TensorGemm(implicit val p: Parameters) extends TensorGemmOrig
// Works on WgtTest; works on AccTest (bypass added)
