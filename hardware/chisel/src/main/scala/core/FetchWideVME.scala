...
//  val inst_q = Module(new SyncQueue2PortMem(UInt(INST_BITS.W), entries_q))
  ...
  // this queue is used to decouple vme from fetch over distance by 1 stage
  //val pipeDelayQueueDeq = Queue(io.vme_rd.data, 2)
  //val pipeDelayQueueDeqV = pipeDelayQueueDeq.valid
  //val pipeDelayQueueDeqF = pipeDelayQueueDeq.fire()
  //val pipeDelayQueueDeqB = pipeDelayQueueDeq.bits
  //pipeDelayQueueDeq.ready := true.B
  ...
  for (i <- 0 until tensorsInClNb) {
    if (p(VerifKey).trace) {
      when(state =/= sIdle) {
        when (wmask(i) && io.vme_rd.data.valid) {
  ...
  if (p(VerifKey).trace) {
    when(io.inst.ld.fire()) {
      trace_mgr.Event("FETCH", "LD   %x\n", dec.io.inst)
    }
    when(io.inst.co.fire()) {
      trace_mgr.Event("FETCH", "CP   %x\n", dec.io.inst)
    }
    when(io.inst.st.fire()) {
      trace_mgr.Event("FETCH", "ST   %x\n", dec.io.inst)
    }
  }
